...
ReScanCtidScan(CustomScanState *node)
{
	...
	{
		ExprState  *fexstate = (ExprState *) lfirst(lc);
		OpExpr		   *op = (OpExpr *)fexstate->expr;
		...
		{
			constvar = (ExprState *) lsecond(op->args);
			opno = op->opno;
		}
		else if (IsCTIDVar(arg2, scanrelid))
		{
			constvar = (ExprState *) linitial(op->args);
			opno = get_commutator(op->opno);
		}
		else
			elog(ERROR, "could not identify CTID variable");

		isnull = constvar->constisnull;
		itemptr = (ItemPointer)constvar->constvalue;
		// itemptr = (ItemPointer)
		// 	DatumGetPointer(ExecEvalExprSwitchContext(constvar,
		// 											  econtext,
													//   &isnull));
		if (isnull)
		{
	...
	if (has_ubound && has_lbound)
		scan = heap_beginscan(relation, estate->es_snapshot, 2, &keys[0], NULL, flags);
	else if (has_ubound)
		scan = heap_beginscan(relation, estate->es_snapshot, 1, &keys[0], NULL, flags);
	else if (has_lbound)
		scan = heap_beginscan(relation, estate->es_snapshot, 1, &keys[1], NULL, flags);
	else
		scan = heap_beginscan(relation, estate->es_snapshot, 0, NULL, NULL, flags);
	...
	if (direction == ForwardScanDirection && has_lbound)
	{
		BlockNumber	blknum = Min(BlockIdGetBlockNumber(&ip_min.ip_blkid),
								 ((HeapScanDesc)scan)->rs_nblocks - 1);
		((HeapScanDesc)scan)->rs_startblock = blknum;
	}
...
ExplainCtidScan(CustomScanState *node, List *ancestors, ExplainState *es)
{
	...
	if (cscan->custom_exprs)
	{
		...
		context = set_deparse_context_planstate(es->deparse_cxt, (Node *)&node->ss.ps, ancestors);
...
_PG_init(void)
{
	...
	RegisterCustomScanMethods(&ctidscan_scan_methods);
}
